...
namespace PhysicsBasedCharacterController
{
    ...
    public class CharacterManager : MonoBehaviour
    {
        ...
        private Vector3 newForward;  // Added by Matt Bart 15/03
        private float newTargetAngle; // Added by Matt Bart 15/03
        ...
        {
            // I want to make the character to move in directions referenced by the camera
            if (axisInput.magnitude > movementThreshold)
            {
                // My shitty code
                newForward = new Vector3(axisInput.x, 0, axisInput.y);
                newForward = axisInput.x * characterCamera.transform.right.normalized + axisInput.y * characterCamera.transform.forward.normalized;
                newForward.y = 0f;
                // End my shitty code
                
                targetAngle = Mathf.Atan2(axisInput.x, axisInput.y) * Mathf.Rad2Deg + characterCamera.transform.eulerAngles.y;
                newTargetAngle = characterCamera.transform.eulerAngles.y;

                if (!sprint || isCrouch) rigidbody.velocity = Vector3.SmoothDamp(rigidbody.velocity, newForward * movementSpeed, ref currVelocity, dampSpeedUp); // if sprinting
                else rigidbody.velocity = Vector3.SmoothDamp(rigidbody.velocity, newForward * sprintSpeed, ref currVelocity, dampSpeedUp); // normal walk speed
        ...
        {
            //I changed targetAngle variables to newTargetAngles
            float angle = Mathf.SmoothDampAngle(characterModel.transform.eulerAngles.y, newTargetAngle, ref turnSmoothVelocity, characterModelRotationSmooth);
            transform.rotation = Quaternion.Euler(0f, newTargetAngle, 0f);
